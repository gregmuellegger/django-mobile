...
class TemplateLoaderTests(BaseTestCase):
    def test_load_template_on_filesystem(self):
        ...
        def testing(filesystem_loader, app_directories_loader, flavour=None):
            ...
            try:
                ...
                pass
            self.assertEqual(filesystem_loader.call_args[0][0], f'{flavour}/base.html')
            self.assertEqual(app_directories_loader.call_args[0][0], f'{flavour}/base.html')

        testing(flavour='mobile')
        testing(flavour='full')
    ...
    def test_get_template_sources_on_filesystem(self):
        ...
        def testing(filesystem_loader, app_directories_loader, flavour=None):
            filesystem_loader.return_value = iter([])
            app_directories_loader.return_value = iter([])
            ...
            set_flavour(flavour)
            try:
                for _ in loader.get_template_sources('base.html'):
                    pass
            except RuntimeError:
                pass
            self.assertEqual(filesystem_loader.call_args[0][0], f'{flavour}/base.html')
    ...
    def test_functional_mobile(self):
        set_flavour('mobile')
        result = render_to_string('index.html')
        result = result.strip()
...
class MobileDetectionMiddlewareTests(BaseTestCase):
    ...
    def test_mobile_browser_agent(self, set_flavour):
        ...
        middleware = MobileDetectionMiddleware(lambda: None)
...
class SetFlavourMiddlewareTests(BaseTestCase):
    def test_set_default_flavour(self):
        ...
        request.GET = {}
        middleware = SetFlavourMiddleware(lambda: None)
        middleware.process_request(request)
